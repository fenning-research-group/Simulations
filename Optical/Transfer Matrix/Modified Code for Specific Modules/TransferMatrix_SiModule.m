...
function TransferMatrix_SiModule
%------------BEGIN USER INPUT PARAMETERS SPECITIFCATION---------------
%
lambda=1000:1500; % Wavelengths over which field patterns are calculated
stepsize = 10;   % The electric field is calculated at a latice of points (nm)
...
% are in nanometers.
plotWavelengths = [1400 1450 1500];
...
% layers = {'lowFeglass' 'EVAdry' 'SiNx' 'Si' 'AlSi'}; % Names of layers of materials starting from side light is incident from
% thicknesses = [3000000 450000 70 160000 2000];  % thickness of each corresponding layer in nm (thickness of the first layer is irrelivant)

layers = fliplr({'SiNx' 'Si' 'AlSi'}); % Names of layers of materials starting from side light is incident from
thicknesses = fliplr([70 160000 2000]);  % thickness of each corresponding layer in nm (thickness of the first layer is irrelivant)
...
plotGeneration = true;
activeLayer = 3; % index of material layer where photocurrent is generated
%
...
if plotGeneration == true
    ...
    AM15=interp1(AM15_data(:,1), AM15_data(:,2)*concentration, lambda, 'linear', 'extrap');
...
[IndRefr,IndRefr_names]=xlsread('Index_of_Refraction_library_SiModule.xls');
